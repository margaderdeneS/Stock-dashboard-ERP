...
class SaleOrderLine(models.Model):
    ...
    def get_product_information(self, order_by, direct, info_compare_type, datestart=None, dateto=None):
        ...
        if datestart and dateto:
            where += 'WHERE sale.date_order BETWEEN %s AND %s' % (datestart, dateto)
        products_query = """SELECT DISTINCT product_id, pt.name, TRUNC(pt.list_price, 2), pp.default_code, TRUNC(SUM(discount), 2) as discount, 
            TRUNC(AVG(price_unit), 2) as avg_price, TRUNC(SUM(price_subtotal), 2) as no_tax_price, TRUNC(SUM(price_unit * product_uom_qty), 2) as price, 
            TRUNC(SUM(price_unit * product_uom_qty) - SUM(price_subtotal), 2) as tax_price, TRUNC(SUM(product_uom_qty), 2) as quantity 
            FROM sale_order_line 
            LEFT JOIN product_product pp ON pp.id = product_id 
            LEFT JOIN product_template pt ON pt.id = pp.product_tmpl_id 
            LEFT JOIN product_category pc ON pc.id = pt.categ_id 
            LEFT JOIN sale_order sale ON sale.id = order_id 
            """+where+"""
